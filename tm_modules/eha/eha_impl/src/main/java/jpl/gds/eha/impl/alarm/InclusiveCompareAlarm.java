 ...
 * for an inclusive range alarm, in which a channel is considered to be in alarm
 * if its value is between the lower and upper thresholds (inclusive). This alarm
 * works only for numeric channels.
 *
 */
public class InclusiveCompareAlarm extends AbstractAlarm {
     ...
     */
	public InclusiveCompareAlarm(final IAlarmDefinition def, final TimeComparisonStrategyContextFlag timeStrategy) {
	...
	protected IAlarmValue calculateAlarm(final IAlarmHistoryProvider history, final IServiceChannelValue channelVal)
	{
		...
		if (checkValue >= this.definition.getLowerLimit() && checkValue <= this.definition.getUpperLimit()) {
