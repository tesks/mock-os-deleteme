 ...
 * generate pre-channelized packets using a defined list of channels per APID.
 * It creates the portion of an pre-channelized packet from the end of the
 * secondary header onwards. It must be supplied a seed object that tells it
 * which channels and APIDs to generate.
 * 
 *
 */
public class ByApidChannelBodyGenerator extends AbstractChannelBodyGenerator
		implements ISeededGenerator {

	private Map<Integer, List<String>> channelsPerApid;

	/**
	 * Basic constructor.
	 */
	public ByApidChannelBodyGenerator() {
	 ...
	 */
	public ByApidChannelBodyGenerator(final TruthFile truthFile) {
	...
	public void setSeedData(final ISeedData seed)
		...
		this.channelsPerApid = this.seedData.getChannelsPerApid();
	}
	...
	public Object getNext() {
		...
		return new Pair<Integer, byte[]>(apid,
				getChannelBody(getDefsForApid(apid)));
	}
	...
	public Object getRandom() {
		...
		throw new UnsupportedOperationException(
				"Random channel body generation is not supported in BY_APID mode");
	}
	 ...
	 */
	private List<IChannelDefinition> getDefsForApid(final int apid) {

		/*
		 * Get the list of desired channels for this APID.
		 */
		final List<String> desiredChans = this.channelsPerApid.get(apid);
