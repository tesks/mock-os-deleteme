/*
 * Copyright 2006-2018. California Institute of Technology.
 * ALL RIGHTS RESERVED.
 * U.S. Government sponsorship acknowledged.
 *
 * This software is subject to U. S. export control laws and
 * regulations (22 C.F.R. 120-130 and 15 C.F.R. 730-774). To the
 * extent that the software is subject to U.S. export control laws
 * and regulations, the recipient has the responsibility to obtain
 * export licenses or other export authority as may be required
 * before exporting such information to foreign countries or
 * providing access to foreign nationals.
 */
package jpl.gds.shared.time;

import java.util.Map;

import jpl.gds.shared.sys.SystemUtilities;
import jpl.gds.shared.types.BitBuffer;

/**
 * An ISclkExtractor implementation based upon the project default SCLK.
 * 
 *
 */
public class ProjectSclkExtractor implements ISclkExtractor {

	// TODO this class should be deleted once the time adaptation schema is done and merged with the frame schema
	final ISclkExtractor sclkExtract = TimeProperties.getInstance().getCanonicalExtractor();

    /**
     * @{inheritDoc}
     * @see jpl.gds.shared.time.ISclkExtractor#getValueFromBytes(byte[], int)
     */
    @Override
    public ISclk getValueFromBytes(final byte[] buff, final int startingOffset) {
    	return sclkExtract.getValueFromBytes(buff, startingOffset);
    }

    @Override
    public void setStaticArgs(final Map<String, Object> params) {
        SystemUtilities.doNothing();
        
    }

	@Override
	public boolean hasEnoughBytes(final byte[] buff, final int startingOffset) {
		return sclkExtract.hasEnoughBytes(buff, startingOffset);
	}

	@Override
    public ISclk getValueFromBits(final BitBuffer buffer, final Map<String, Object> args) {
		return sclkExtract.getValueFromBits(buffer, args);
	}

}
