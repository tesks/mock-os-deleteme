 ...
 * unsigned integer. By definition, this parser is a validating parser. Always
 * validates lower bound, which defaults to 0. Optionally, can be supplied with
 * an upper bound.
 * 
 */
public class UnsignedIntOptionParser extends AbstractRangeCheckingOptionParser<UnsignedInteger> {
    
    /**
     * Constructor for an unsigned integer parser with default range.
     */
    public UnsignedIntOptionParser() {
        super();
    }

    /**
     * Constructor for an unsigned integer parser with
     * default lower bound (0) and specified upper bound.
     * 
     * @param highestValue the maximum allowed value (inclusive)
     */
    public UnsignedIntOptionParser(UnsignedInteger highestValue) {
        super();
        setMaxValue(highestValue);
    }
    
    /**
     * Constructor for an unsigned integer parser with restricted range.
     * 
     * @param lowestValue the maximum allowed value (inclusive)
     * @param highestValue the maximum allowed value (inclusive)
     */
    
    public UnsignedIntOptionParser(UnsignedInteger lowestValue, UnsignedInteger highestValue) {
     ...
     */
    @Override
    public UnsignedInteger parse(ICommandLine commandLine, ICommandLineOption<UnsignedInteger> opt) throws ParseException {
        ...
        UnsignedInteger unsignedValue = null;
        try {
            unsignedValue = UnsignedInteger.valueOf(value);
     ...
     */
    public void setMaxValue(UnsignedInteger highestValue) {
       
        if (highestValue == null) {
            super.setMaxValue(UnsignedInteger.MAX_VALUE);
        } else {
     ...
     */
    public void setMinValue(UnsignedInteger lowestValue) {
        if (lowestValue == null) {
            super.setMinValue(UnsignedInteger.MIN_VALUE);
        } else {
