...
/**
 * A class that represents a command line option that takes a float value.
 * Optionally, the option value may also have a restricted range.
 * 
 *
 */
public class FloatOption extends CommandLineOption<Float> {

    private static final long serialVersionUID = 1L;

    /**
     * Constructor for a float option with restricted range. 
     * 
     * @param shortOpt
     ...
     *            for no upper bound
     */
    public FloatOption(final String shortOpt, final String longOpt,
            final String argName, final String description,
            final boolean required, Float minValue, Float maxValue) {
        super(shortOpt, longOpt, true, argName, description, required,
                new FloatOptionParser(minValue, maxValue));
    }

    /**
     * Constructor for a float option with default range.
     * 
     * @param shortOpt
     *            short option name; may be null
     * @param longOpt
     ...
     */
    public FloatOptionParser getParser() {
        return (FloatOptionParser) parser;
    }
