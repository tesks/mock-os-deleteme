...
/**
 * This is the database read/retrieval interface to the CfdpFileUplinkFinished table in
 ...
 *
 */
public class CfdpFileUplinkFinishedFetch extends ACfdpMySqlFetch implements ICfdpFileUplinkFinishedFetch {

    protected String getDbTable() {
        return ICfdpFileUplinkFinishedLDIStore.DB_CFDP_FILE_UPLINK_FINISHED_DATA_TABLE_NAME;
    }

    private final IDbCfdpFileUplinkFinishedFactory dbCfdpFileUplinkFinishedFactory;

    /**
     * Creates an instance of CfdpFileUplinkFinishedFetch.
     *
     ...
     */
    public CfdpFileUplinkFinishedFetch(final ApplicationContext appContext, final boolean printSqlStmt) {
        super(appContext, printSqlStmt);
        dbCfdpFileUplinkFinishedFactory = appContext.getBean(IDbCfdpFileUplinkFinishedFactory.class);
    }
    ...
    public List<? extends IDbRecord> get(final IDbContextInfoProvider tsi, DatabaseTimeRange range, final int batchSize,
        ...
        if (dbProperties.getUseDatabase() == false) {
            return (new ArrayList<IDbCfdpFileUplinkFinishedProvider>());
        }

        return super.get(DB_CFDP_FILE_UPLINK_FINISHED_TABLE_ABBREV, tsi, range, batchSize, useContext, params);
    }
    ...
    protected List<? extends IDbRecord> populateAndExecute(final IDbContextInfoProvider tsi,
        ...
        try {
            ...
            String selectClause = queryConfig.getQueryClause(clauseType,
                    getActualTableName(ICfdpFileUplinkFinishedLDIStore.DB_CFDP_FILE_UPLINK_FINISHED_DATA_TABLE_NAME));
            ...
            throw new DatabaseException("Error retrieving CFDP File Uplink Finished events from database: " + e.getMessage(), e);
        }
     ...
     */
    @Override
    public List<IDbCfdpFileUplinkFinishedProvider> getNextResultBatch() throws DatabaseException {
        return getResults();
    }
     ...
     *
     * @return The list of CFDP File Uplink Finished events that is part of the results of
     * a query executed using the "get(...)" methods. When an empty list is returned,
     * it means no more results are left.
     * @throws DatabaseException If there happens to be an exception retrieving the next set of
     *                           results
     */
    @Override
    protected List<IDbCfdpFileUplinkFinishedProvider> getResults() throws DatabaseException {
        final List<IDbCfdpFileUplinkFinishedProvider> refs = new ArrayList<IDbCfdpFileUplinkFinishedProvider>();
        ...
        try {
            ...
            while (count < this.batchSize) {
                ...
                // pull out all the information for this CFDP File Uplink Finished event and add it
                // to the return list
                final IDbCfdpFileUplinkFinishedUpdater dp = dbCfdpFileUplinkFinishedFactory.createQueryableUpdater();
                ...
                dp.setUplinkFileMetadataFileLocation(this.results.getString("uplinkFileMetadataFileLocation"));
                dp.setUplinkFileLocation(this.results.getString("uplinkFileLocation"));
