...
/**
 * This is the database read/retrieval interface to the CfdpPduReceived table in
 ...
 *
 */
public class CfdpPduReceivedFetch extends ACfdpMySqlFetch implements ICfdpPduReceivedFetch {

    protected String getDbTable() {
        return ICfdpPduReceivedLDIStore.DB_CFDP_PDU_RECEIVED_DATA_TABLE_NAME;
    }

    private final IDbCfdpPduReceivedFactory dbCfdpPduReceivedFactory;

    /**
     * Creates an instance of CfdpPduReceivedFetch.
     *
     ...
     */
    public CfdpPduReceivedFetch(final ApplicationContext appContext, final boolean printSqlStmt) {
        super(appContext, printSqlStmt);
        dbCfdpPduReceivedFactory = appContext.getBean(IDbCfdpPduReceivedFactory.class);
    }
    ...
    public List<? extends IDbRecord> get(final IDbContextInfoProvider tsi, DatabaseTimeRange range, final int batchSize,
        ...
        if (dbProperties.getUseDatabase() == false) {
            return (new ArrayList<IDbCfdpPduReceivedProvider>());
        }

        return super.get(DB_CFDP_PDU_RECEIVED_TABLE_ABBREV, tsi, range, batchSize, useContext, params);
    }
    ...
    protected List<? extends IDbRecord> populateAndExecute(final IDbContextInfoProvider tsi,
        ...
        try {
            ...
            String selectClause = queryConfig.getQueryClause(clauseType,
                    getActualTableName(ICfdpPduReceivedLDIStore.DB_CFDP_PDU_RECEIVED_DATA_TABLE_NAME));
            ...
            throw new DatabaseException("Error retrieving CFDP PDU Received from database: " + e.getMessage(), e);
        }
     ...
     */
    @Override
    public List<IDbCfdpPduReceivedProvider> getNextResultBatch() throws DatabaseException {
        return getResults();
    }
     ...
     *
     * @return The list of CFDP PDU Received events that is part of the results of a query executed
     ...
     */
    @Override
    protected List<IDbCfdpPduReceivedProvider> getResults() throws DatabaseException {
        final List<IDbCfdpPduReceivedProvider> refs = new ArrayList<IDbCfdpPduReceivedProvider>();
        ...
        try {
            ...
            while (count < this.batchSize) {
                ...
                // pull out all the information for this CFDP PDU Received and add it
                // to the return list
                final IDbCfdpPduReceivedUpdater dp = dbCfdpPduReceivedFactory.createQueryableUpdater();
            ...
            throw new DatabaseException("Error retrieving CFDP PDU Received events from database: " + e.getMessage(), e);
        }
