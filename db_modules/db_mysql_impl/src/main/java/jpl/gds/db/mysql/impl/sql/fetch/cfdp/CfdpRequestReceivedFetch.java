...
/**
 * This is the database read/retrieval interface to the CfdpRequestReceived table in
 ...
 *
 */
public class CfdpRequestReceivedFetch extends ACfdpMySqlFetch implements ICfdpRequestReceivedFetch {

    protected String getDbTable() {
        return ICfdpRequestReceivedLDIStore.DB_CFDP_REQUEST_RECEIVED_DATA_TABLE_NAME;
    }

    private final IDbCfdpRequestReceivedFactory dbCfdpRequestReceivedFactory;

    /**
     * Creates an instance of CfdpRequestReceivedFetch.
     *
     ...
     */
    public CfdpRequestReceivedFetch(final ApplicationContext appContext, final boolean printSqlStmt) {
        super(appContext, printSqlStmt);
        dbCfdpRequestReceivedFactory = appContext.getBean(IDbCfdpRequestReceivedFactory.class);
    }
    ...
    public List<? extends IDbRecord> get(final IDbContextInfoProvider tsi, DatabaseTimeRange range, final int batchSize,
        ...
        if (dbProperties.getUseDatabase() == false) {
            return (new ArrayList<IDbCfdpRequestReceivedProvider>());
        }

        return super.get(DB_CFDP_REQUEST_RECEIVED_TABLE_ABBREV, tsi, range, batchSize, useContext, params);
    }
    ...
    protected List<? extends IDbRecord> populateAndExecute(final IDbContextInfoProvider tsi,
        ...
        try {
            ...
            String selectClause = queryConfig.getQueryClause(clauseType,
                    getActualTableName(ICfdpRequestReceivedLDIStore.DB_CFDP_REQUEST_RECEIVED_DATA_TABLE_NAME));
            ...
            throw new DatabaseException("Error retrieving CFDP Request Received from database: " + e.getMessage(), e);
        }
     ...
     */
    @Override
    public List<IDbCfdpRequestReceivedProvider> getNextResultBatch() throws DatabaseException {
        return getResults();
    }
     ...
     *
     * @return The list of CFDP Request Received events that is part of the results of a query executed
     ...
     */
    @Override
    protected List<IDbCfdpRequestReceivedProvider> getResults() throws DatabaseException {
        final List<IDbCfdpRequestReceivedProvider> refs = new ArrayList<IDbCfdpRequestReceivedProvider>();
        ...
        try {
            ...
            while (count < this.batchSize) {
                ...
                // pull out all the information for this CFDP Request Received and add it
                // to the return list
                final IDbCfdpRequestReceivedUpdater dp = dbCfdpRequestReceivedFactory.createQueryableUpdater();
                ...
                dp.setRequesterId(this.results.getString("requesterId"));
                dp.setHttpUser(this.results.getString("httpUser"));
                dp.setHttpHost(this.results.getString("httpHost"));
                dp.setRequestContent(this.results.getString("requestContent"));
            ...
            throw new DatabaseException("Error retrieving CFDP Request Received events from database: " + e.getMessage(), e);
        }
