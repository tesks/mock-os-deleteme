...
def test():
    ...
    #Get the current value of channel TLM1-1234 and check if it has a DN value of 1
    chanval = downlinkProxy.channelValueTable.getById('TLM1-1234')
    if chanval:
        if chanval.dn != 1:
            logger.error('Channel ID TLM1-1234 did not have a value of 1')
    else:
        logger.error('Did not find a value for the channel TLM1-1234')

    #Get all the received EVRs with event ID 220
    evr = downlinkProxy.evrList.getByEventId(7602)
    if not evr:
        logger.error('Could not find the EVR with event ID 220')
    elif len(evr) > 1:
        logger.warn('Found multiple evrs with event ID 220')
