...
public class CfdpPduReceivedFetchApp extends ACfdpSubFetchApp {

	private static final String APP_NAME = ApplicationConfiguration.getApplicationName("chill_get_cfdp pdurecv");

	private static final int NUM_QUERY_PARAMS = 1;

	public CfdpPduReceivedFetchApp() {
		super(IDbTableNames.DB_CFDP_PDU_RECEIVED_DATA_TABLE_NAME, APP_NAME, "CfdpPduReceivedQuery");
	...
	public IDbSqlFetch getFetch(boolean sqlStmtOnly) {
		fetch = appContext.getBean(IDbSqlFetchFactory.class).getCfdpPduReceivedFetch(sqlStmtOnly);
		return fetch;
	}
	...
	public String getUsage() {
		return(APP_NAME + " " + CfdpFetchApp.AvailableCommandsForHelp.PDURECV + " [options]\n");
	}
