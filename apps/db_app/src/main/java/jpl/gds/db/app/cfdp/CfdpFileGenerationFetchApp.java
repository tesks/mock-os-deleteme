...
public class CfdpFileGenerationFetchApp extends ACfdpSubFetchApp {

	private static final String APP_NAME = ApplicationConfiguration.getApplicationName("chill_get_cfdp filegen");

	private static final int NUM_QUERY_PARAMS = 1;

	public CfdpFileGenerationFetchApp() {
		super(IDbTableNames.DB_CFDP_FILE_GENERATION_DATA_TABLE_NAME, APP_NAME, "CfdpFileGenerationQuery");
	...
	public IDbSqlFetch getFetch(boolean sqlStmtOnly) {
		fetch = appContext.getBean(IDbSqlFetchFactory.class).getCfdpFileGenerationFetch(sqlStmtOnly);
		return fetch;
	}
	...
	public String getUsage() {
        return(APP_NAME + " " + CfdpFetchApp.AvailableCommandsForHelp.FILEGEN + " [options]\n");
	}
