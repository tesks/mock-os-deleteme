...
import jpl.gds.watcher.responder.app.ChannelSampleWatcherApp;
import jpl.gds.watcher.responder.app.MessageResponderApp;

/**
 * ChannelMessageHandler is the MessageHandler implementation used by the
 * message responder application to listen for EhaChannel messages and put out
 * channel value change information to the console.
 */
public class ChannelSampleMessageHandler extends AbstractMessageHandler {
    ...
    private ChannelSampleWatcherApp appHelper;
    ...
    /**
     * Creates an instance of ChannelSampleMessageHandler.
     * @param appContext the current application context
     */
    public ChannelSampleMessageHandler(final ApplicationContext appContext) {
    	super(appContext);
        ...
        try {
            ...
            outputTemplate = templateManager.getTemplateForStyle(MessageRegistry.getMessageConfig(
                    EhaMessageType.AlarmedEhaChannel), DEFAULT_TEMPLATE);
    ...
    public synchronized void handleMessage(final IExternalMessage m) {
        try {
            ...
            if (messages != null) {
                for (int i = 0; i < messages.length && !isShutdown; i++) {
                    ...
                    if (currentMsg.isType(EhaMessageType.AlarmedEhaChannel)) {
                    	if (ehaMessageCount == 0 && outputTemplate != null) {
                        ...
                        writeError("ChannelSampleMessageHandler got an unrecognized message type: "
                                + currentMsg.getType());
                        continue;
                    }
                }
            ...
            writeError("ChannelSampleMessageHandler could not process message: "
                    + e.toString(), e);
        }
    }
    ...
    private void startContext(final IContextMessage message) {
    	...
    	if (awaitingFirstContext && newConfig instanceof IContextConfiguration) {
	    	...
	    	try {
	    		...
	    		writeLog("ChannelChangeMessageHandler could not load context channel dictionary");
	    	}
    ...
    private synchronized void handleChannelMessage(
            final IAlarmedChannelValueMessage message) {
        ehaMessageCount++;
        writeLog("ChannelSampleMessageHandler got EHA message: "
    ...
    public synchronized void shutdown() {
        ...
        writeLog("ChannelSampleMessageHandler is shutting down");
    }
    ...
    public synchronized void handleEndOfContext(final IEndOfContextMessage m) {
        ...
        if (tc == null || tc.getNumber() == null) {
            writeLog("ChannelSampleMessageHandler received End of Context message for an unknown context; skipping");
            return;
        }
        writeLog("ChannelSampleMessageHandler received End of Context message for context "
                + tc.getNumber());
    ...
    public synchronized void setAppHelper(final IResponderAppHelper app) {
        appHelper = (ChannelSampleWatcherApp) app;
        ...
        try {
            outputTemplate = templateManager.getTemplateForStyle(
                    MessageRegistry.getMessageConfig(EhaMessageType.AlarmedEhaChannel), templateName);
            if (outputTemplate == null) {
