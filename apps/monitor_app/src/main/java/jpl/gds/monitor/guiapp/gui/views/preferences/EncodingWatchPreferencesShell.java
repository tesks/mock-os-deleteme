...
/**
 * EncodingWatchPreferencesShell is the SWT GUI preferences window class for the 
 * encoding watch view.
 *
 */
public class EncodingWatchPreferencesShell extends AbstractViewPreferences implements ViewPreferencesShell {
    /**
     * Encoding watch preferences window title
     */
    public static final String TITLE = "Encoding Watch Preferences";
    
    private final Shell parent;
    private TableComposite tableConfigurer;
    private ChillTable encodingTable;
     ...
     */
    public EncodingWatchPreferencesShell(final ApplicationContext appContext, final Shell parent) {
        super(appContext, TITLE);
        this.parent = parent;
        this.trace = TraceManager.getTracer(appContext, Loggers.DEFAULT);
        createControls();
        //this.prefShell.setSize(410, 560);
    ...
    protected void createControls() {
            ...
            public void widgetSelected(final org.eclipse.swt.events.SelectionEvent e) {
                try {
                    if (tableConfigurer.getTable().getActualColumnCount() == 0) {
                        SWTUtilities.showMessageDialog(EncodingWatchPreferencesShell.this.prefShell, "No Columns",
                                "You cannot disable all the table columns, or the table will be blank.");
                        return;
                    }
                    applyChanges();
                    EncodingWatchPreferencesShell.this.dirty = EncodingWatchPreferencesShell.this.tableConfigurer.getDirty();
                    if (EncodingWatchPreferencesShell.this.dirty) {
                        EncodingWatchPreferencesShell.this.changeColumns = true;
                    }
                    EncodingWatchPreferencesShell.this.prefShell.close();
        ...
        cancelButton.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(final org.eclipse.swt.events.SelectionEvent e) {
                try {
                    EncodingWatchPreferencesShell.this.canceled = true;
                    EncodingWatchPreferencesShell.this.prefShell.close();
                } catch (final Exception e1) {
                    e1.printStackTrace();
                }
    ...
    /**
     * Determines if the selected columns to be displayed have changed
     * 
     * @return true if column selection has changed, false otherwise
     */
    ...
    public void setValuesFromViewConfiguration(final IViewConfiguration config) {
        ...
        final EncodingViewConfiguration chanConfig = (EncodingViewConfiguration)config;
        this.encodingTable = chanConfig.getTable(EncodingViewConfiguration.ENCODING_TABLE_NAME);
        if (this.tableConfigurer != null && !this.tableConfigurer.getComposite().isDisposed()) {
            this.tableConfigurer.init(this.encodingTable);
