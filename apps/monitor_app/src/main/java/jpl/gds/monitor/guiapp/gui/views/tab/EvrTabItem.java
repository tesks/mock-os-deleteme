...
/**
 * 
 * EvrTabItem is the tab item container for EvrComposite in the non-RCP version
 * of the monitor.  It plays a ViewTab role, so it can be added to or removed
 * from the monitor by generic view logic.
 *
 */
public class EvrTabItem extends AbstractTabItem {
    /**
     * EVR tab item title
     */
     ...
     */
    protected EvrComposite evrComp;
    
    /**
     * Creates an instance of EvrTabItem.
     * @param config the EvrViewConfiguration object containing display settings
     */
    public EvrTabItem(final ApplicationContext appContext, final IViewConfiguration config) {
        super(config);
        this.evrComp = new EvrComposite(appContext, this.viewConfig);
     ...
     */
    public EvrTabItem(final ApplicationContext appContext) {
        this(appContext, new EvrViewConfiguration(appContext));
    }
    ...
    protected void createControls() {
        ...
        this.tab.addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(final DisposeEvent event) {
                try {
                    if (EvrTabItem.this.evrComp != null) {
                    	EvrTabItem.this.evrComp.getMainControl().dispose();
                    }
